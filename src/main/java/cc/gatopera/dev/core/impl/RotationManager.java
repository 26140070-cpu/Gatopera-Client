...
import cc.gatopera.dev.api.events.impl.*;
...
import cc.gatopera.dev.mod.modules.impl.client.BaritoneModule;
import cc.gatopera.dev.mod.modules.impl.client.AntiCheat;
import cc.gatopera.dev.mod.modules.impl.movement.MoveFix;
...
public class RotationManager implements Wrapper {
    ...
    public void snapBack() {
        mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(mc.player.getX(), mc.player.getY(), mc.player.getZ(), Gatopera.ROTATION.rotationYaw, Gatopera.ROTATION.rotationPitch, mc.player.isOnGround()));
    }
    ...
    public void snapAt(Vec3d directionVec) {
        ...
        if (AntiCheat.INSTANCE.noSpamRotation.getValue()) {
            if (MathHelper.angleBetween(angle[0], Gatopera.ROTATION.lastYaw) < AntiCheat.INSTANCE.fov.getValueFloat() && Math.abs(angle[1] - Gatopera.ROTATION.lastPitch) < AntiCheat.INSTANCE.fov.getValueFloat()) {
                return;
            }
        }
    ...
    public void update(MovementPacketsEvent event) {
        if (MoveFix.INSTANCE.isOn() && !BaritoneModule.isActive()) {
            ...
            Gatopera.EVENT_BUS.post(event1);
    ...
    private void updateNext() {
        ...
        Gatopera.EVENT_BUS.post(rotateEvent);
    ...
    public void onLastRotation(RotateEvent event) {
        ...
        Gatopera.EVENT_BUS.post(lookAtEvent);
