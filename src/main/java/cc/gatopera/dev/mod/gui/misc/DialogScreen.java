...
public class DialogScreen extends Screen implements Wrapper {
    ...
    public void render(@NotNull DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        drawText(context, yesText, mainX + 60, mainY + 110, yesH ? 0xFFFFFFFF : 0xFFCCCCCC);
        drawText(context, noText, mainX + 180, mainY + 110, noH ? 0xFFFFFFFF : 0xFFCCCCCC);
    ...
    private void drawText(DrawContext context, String text, float x, float y, int color) {
        ...
        if (useCustom) {
            try {
                FontRenderers.ui.drawCenteredString(context.getMatrices(), text, x, y, color);
                return;
            } catch (Throwable ignored) {
            }
        }
        context.drawCenteredTextWithShadow(this.textRenderer, Text.of(text), (int) x, (int) y, color);
    }
