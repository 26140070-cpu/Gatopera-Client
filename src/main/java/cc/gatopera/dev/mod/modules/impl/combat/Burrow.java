...
public class Burrow extends Module {
    ...
    public void onUpdate() {
        if (Gatopera.PLAYER.isInWeb(mc.player)) {
            webTimer.reset();
            return;
        }
        ...
        if (this.rotate.getValue() == RotateMode.Bypass) {
            Gatopera.ROTATION.snapAt(Gatopera.ROTATION.rotationYaw, 90);
        }
    ...
    private void gotoPos(BlockPos offPos) {
        ...
        if (rotate.getValue() == RotateMode.None) {
            ...
            mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(offPos.getX() + 0.5, mc.player.getY() + 0.1, offPos.getZ() + 0.5, Gatopera.ROTATION.rotationYaw, 90, false));
        }
    }
    ...
    private boolean canPlace(BlockPos pos) {
        ...
        if (detectMine.getValue() && Gatopera.BREAK.isMining(pos)) {
            return false;
        }
