...
public class TimerModule extends Module {
	...
	public void onRender2D(DrawContext drawContext, float tickDelta) {
		...
		if (MovementUtil.isMoving() && !Gatopera.PLAYER.insideBlock) {
			...
			if (timer2.passed(lastMs)) {
				...
				if (smooth.getValue()) {
					double timer = Gatopera.TIMER.getDefault() + (1 - end.ease(ease.getValue())) * (shiftTimer.getValueFloat() - 1) * (lastMs / accumulate.getValue());
					Gatopera.TIMER.set((float) Math.max(Gatopera.TIMER.getDefault(), timer));
				} else {
					Gatopera.TIMER.set(shiftTimer.getValueFloat());
				}
			}
		} else {
			if (moving) {
