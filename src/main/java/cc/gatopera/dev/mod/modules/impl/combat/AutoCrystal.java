...
import cc.gatopera.dev.api.utils.math.*;
import cc.gatopera.dev.api.utils.render.ColorUtil;
import cc.gatopera.dev.api.utils.render.JelloUtil;
...
import cc.gatopera.dev.asm.accessors.IEntity;
...
import cc.gatopera.dev.mod.modules.impl.render.ExplosionSpawn;
...
public class AutoCrystal extends Module {
    ...
    private void doBreak(BlockPos pos) {
        ...
        for (EndCrystalEntity entity : BlockUtil.getEndCrystals(new Box(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1, pos.getY() + 2, pos.getZ() + 1))) {
            ...
            if (crystalPos != null && displayTarget != null && lastDamage >= getDamage(displayTarget) && afterBreak.getValue()) {
                if (!yawStep.getValue() || !checkFov.getValue() || Gatopera.ROTATION.inFov(entity.getPos(), fov.getValueFloat())) {
                    doPlace(crystalPos);
                }
            }
    ...
    private boolean faceVector(Vec3d directionVec) {
        if (!yawStep.getValue()) {
            ...
            if (Gatopera.ROTATION.inFov(directionVec, fov.getValueFloat())) {
                return true;
            }
        }
