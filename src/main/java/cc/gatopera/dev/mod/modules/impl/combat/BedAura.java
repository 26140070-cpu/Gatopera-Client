...
import cc.gatopera.dev.api.utils.math.ExplosionUtil;
...
public class BedAura extends Module {
	...
	public void onEnable() {
		lastYaw = Gatopera.ROTATION.lastYaw;
		lastPitch = Gatopera.ROTATION.lastPitch;
	}
	...
	public void onRotate(RotateEvent event) {
		if (!rotate.getValue() && yawDeceive.getValue()) {
			...
			float[] newAngle = injectStep(Gatopera.ROTATION.getRotation(directionVec), steps.getValueFloat());
			lastYaw = newAngle[0];
	...
	private void doPlace(BlockPos pos) {
		...
		for (Direction i : Direction.values()) {
			...
			if ((legacy.getValue() && BlockUtil.canReplace(pos.offset(i)) || BlockUtil.clientCanPlace(pos.offset(i), false)) && BlockUtil.canClick(pos.offset(i).down()) && (!checkMine.getValue() || !Gatopera.BREAK.isMining(pos.offset(i)))) {
				facing = i;
				break;
			}
		}
	...
	private boolean isTrueFacing(BlockPos pos, Direction facing) {
		...
		return Direction.fromRotation(Gatopera.ROTATION.getRotation(hitVec)[0]) == facing;
	}

	private boolean isTrueFacingNow(Direction facing) {
		return Direction.fromRotation(Gatopera.ROTATION.lastYaw) == facing;
	}
	public enum Page {
		General,
		Rotate,
	...
	public boolean faceVector(Vec3d directionVec) {
		if (!yawStep.getValue()) {
			Gatopera.ROTATION.lookAt(directionVec);
			return true;
		} else {
			this.directionVec = directionVec;
			if (Gatopera.ROTATION.inFov(directionVec, fov.getValueFloat())) return true;
		}
